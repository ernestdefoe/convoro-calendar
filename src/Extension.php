...
class Extension extends ServiceProvider
{
    ...
    public function boot(): void
    {
        Route::middleware('web')->group(function () {
            ...
            Route::get('/api/ext/events/upcoming', function () {
                $tz = self::tz();
                $rows = DB::table('events')->where('starts_at', '>=', now()->utc()->subHours(3))
                    ->orderBy('starts_at')->limit(6)
                    ->get(['id', 'title', 'starts_at', 'location', 'is_online'])
                    ->map(function ($e) use ($tz) {
                        $d = Carbon::parse($e->starts_at, 'UTC')->setTimezone($tz);
                        $e->day = $d->format('j');
                        $e->mon = $d->format('M');
                        $e->time = $d->format('g:ia');
    ...
    private static function save(Request $request, ?int $id)
    {
        ...
        if ($id === null) {
            $newId = DB::table('events')->insertGetId($fields + [
                'user_id' => $request->user()->id, 'reminded_day' => false, 'reminded_hour' => false, 'created_at' => now(),
            ]);
        ...
        abort_if(! $ev, 404);
        abort_unless(self::canManage($ev), 403);
        // If the start moved, let reminders fire again for the new time.
        if ($fields['starts_at'] !== $ev->starts_at) {
            $fields['reminded_day'] = false;
            $fields['reminded_hour'] = false;
        }
    ...
    private static function listHtml(?object $user, int $cat): string
    {
        ...
        $rows = DB::table('events')->join('users', 'users.id', '=', 'events.user_id')
            ...
            ->where('starts_at', '>=', now()->utc()->subHours(3))
        ...
        $cards = '';
        foreach ($rows as $ev) {
            $start = Carbon::parse($ev->starts_at, 'UTC')->setTimezone($tz);
            $c = self::counts($ev->id);
            $mine = $myRsvps[$ev->id] ?? '';
            ...
            $cards .= '<div class="ev">'
                ...
                .'<div class="body"><div class="trow"><a class="t" href="/events/'.$ev->id.'">'.$e($ev->title).'</a>'.self::catChip($catObj).'</div>'
    ...
    private static function calendarHtml(Request $request, int $cat): string
    {
        ...
        $rows = DB::table('events')->leftJoin('event_categories', 'event_categories.id', '=', 'events.category_id')
            ...
            ->when($cat, fn ($q) => $q->where('events.category_id', $cat))
            ->orderBy('starts_at')->get(['events.id', 'events.title', 'events.starts_at', 'event_categories.color as cat_color']);
        $byDay = [];
        foreach ($rows as $ev) {
            $byDay[(int) Carbon::parse($ev->starts_at, 'UTC')->setTimezone($tz)->day][] = $ev;
        }
...
function openCreate(){EDIT_ID=null;document.getElementById('f_heading').textContent='Create an event';
  sv('f_title','');sv('f_cat','');sv('f_start','');sv('f_end','');sv('f_loc','');sv('f_url','');sv('f_cap','');sv('f_desc','');
...
function openEdit(ev){EDIT_ID=ev.id;document.getElementById('f_heading').textContent='Edit event';
  ...
  sv('f_loc',ev.location);sv('f_url',ev.url);sv('f_cap',ev.capacity);sv('f_desc',ev.description);
...
JS;
    ...
    private static function detailPage(int $id)
    {
        ...
        $user = Auth::user();
        $start = Carbon::parse($ev->starts_at, 'UTC')->setTimezone($tz);
        $end = $ev->ends_at ? Carbon::parse($ev->ends_at, 'UTC')->setTimezone($tz) : null;
        $c = self::counts($id);
        ...
        $evJson = json_encode([
            ...
            'starts' => $start->format('Y-m-d\TH:i'), 'ends' => $end ? $end->format('Y-m-d\TH:i') : '',
        ...
        $body = '<a class="back" href="/events">← All events</a>'
            ...
            .($editBtn ? '<div class="addcal">'.$editBtn.'</div>' : '')
...
.del.solo{width:100%;border:1px solid rgb(var(--c-border));background:transparent;color:#ef4444;border-radius:10px;padding:10px;font-weight:700;cursor:pointer;position:static}
