...
class Extension extends ServiceProvider
{
    ...
    public function boot(): void
    {
        Route::middleware('web')->group(function () {
            Route::get('/events', fn (Request $r) => response(self::indexPage($r)));
            Route::get('/events/{id}.ics', fn (int $id) => self::ical($id))->whereNumber('id');
            Route::get('/events/{id}', fn (int $id) => response(self::detailPage($id)))->whereNumber('id');
    ...
    private static function indexPage(Request $request): string
    {
        $user = Auth::user();
        $e = fn ($v) => htmlspecialchars((string) $v, ENT_QUOTES);
...
FORM;
    ...
    private static function detailPage(int $id): string
    {
        $e = fn ($v) => htmlspecialchars((string) $v, ENT_QUOTES);
...
JS;
    ...
    private static function shell(string $title, string $body, string $css = '', string $js = ''): string
    {
        $theme = Theme::css();
        $palette = Theme::surfacePalette();
        $chrome = Theme::chromeCss();
        $header = Theme::siteHeader(['Events' => '/events']);
...
<meta name="csrf-token" content="{$csrf}"><title>{$title} · {$name}</title>
<style>{$theme}
{$palette}
{$chrome}
{$base}
...
HTML;
    ...
    {
        return <<<'CSS'
*{box-sizing:border-box}body{margin:0;font-family:inherit;background:rgb(var(--c-bg));color:rgb(var(--c-text))}
body{font-family:var(--c-font)}a{color:inherit;text-decoration:none}
